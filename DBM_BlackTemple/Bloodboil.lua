...
Bloodboil.Version	= "1.1";
Bloodboil.Author	= "LYQ";
...
Bloodboil:AddBarOption("Bloodboil #(%d+)")
Bloodboil:AddBarOption("Fel Rage")
Bloodboil:AddBarOption("Normal Phase")
Bloodboil:AddBarOption("Arcing Smash CD")
...
function Bloodboil:OnCombatStart(delay)
	...
	self:StartStatusBarTimer(59.5 - delay, "Fel Rage", "Interface\\Icons\\Spell_Fire_ElementalDevastation");
	self:ScheduleSelf(54.5 - delay, "FelRageWarn");
	self:StartStatusBarTimer(11.5 - delay, "Bloodboil #1", "Interface\\Icons\\Spell_Shadow_BloodBoil");
end
...
function Bloodboil:OnEvent(event, arg1)
    ...
    elseif event == "NormalPhase" then
        self:SendSync("NormalPhase");
	end
end
...
function Bloodboil:OnSync(msg)
	if msg == "Bloodboil" then
        ...
        if boilCounter < 5 then
            self:StartStatusBarTimer(10, "Bloodboil #"..(boilCounter+1), "Interface\\Icons\\Spell_Shadow_BloodBoil");
        end
    elseif msg == "NormalPhase" then
        boilCounter = 0;
        ...
        self:StartStatusBarTimer(11, "Bloodboil #1", "Interface\\Icons\\Spell_Shadow_BloodBoil");
	elseif msg:sub(0, 7) == "FelRage" then
		msg = msg:sub(8);
		self:EndStatusBarTimer("Bloodboil");
		self:StartStatusBarTimer(29.5, "Normal Phase", "Interface\\Icons\\Spell_Nature_WispSplode");
        self:ScheduleSelf(29.5, "NormalPhase");
		self:ScheduleSelf(24, "NormalWarn");
		self:Announce(DBM_BLOODBOIL_WARN_FELRAGE:format(msg), 4);
		boilCounter = 0;
	elseif msg:sub(0, 11) == "ArcingSmash" then
		msg = msg:sub(12);
		self:StartStatusBarTimer(10, "Arcing Smash CD", "Interface\\Icons\\Ability_Warrior_Cleave");
		if self.Options.WarnSmash then
			self:ScheduleSelf(8, "SmashWarn");		
